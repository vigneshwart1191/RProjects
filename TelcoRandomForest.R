...
fit = randomForest(Churn ~ .,data = train,ntree=i)
x1 = c(x1,fit)
}
#to find minimum error rate
which.min(fit$err.rate[,1])
#for 750 gives good accuracy
fit1 = randomForest(Churn ~ .,data = train,ntree=998)
summary(fit1)
plot(fit)
summary(fit)
...
#predict the output 750 trees
Churn2 = predict(fit1,test,type = "class")
table(Churn2,test$Churn)
